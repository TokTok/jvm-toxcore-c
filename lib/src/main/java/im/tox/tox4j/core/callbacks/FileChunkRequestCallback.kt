...
interface FileChunkRequestCallback<ToxCoreState> {
   ...
   * last requested chunk. This happens when a chunk was requested, but the send failed. A seek-back
   * request can occur an arbitrary number of times for any given chunk.
   *
   * In response to receiving this callback, the client should call the function
   * [ [ToxCore.fileSendChunk]] with the requested chunk. If the number of bytes sent through that
   * function is zero, the file transfer is assumed complete. A client must send the full length of
   * data requested with this callback.
   *
   * @param friendNumber The friend number of the receiving friend for this file.
   * @param fileNumber The file transfer identifier returned by { @link
   *   im.tox.tox4j.core.ToxCore#fileSend}.
