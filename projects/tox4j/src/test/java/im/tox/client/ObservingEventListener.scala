...
final class ObservingEventListener[ToxCoreState](next: ToxEventListener[ToxCoreState], observers: ToxEventListener[Unit]*) extends ToxEventAdapter[ToxCoreState] {

  override def friendStatus(friendNumber: Int, status: ToxUserStatus)(state: ToxCoreState): ToxCoreState = {
  ...
  override def friendName(friendNumber: Int, name: ToxNickname)(state: ToxCoreState): ToxCoreState = {
    observers.foreach(_.friendName(friendNumber, name)(()))
    next.friendName(friendNumber, name)(state)
  }

  override def friendMessage(friendNumber: Int, messageType: ToxMessageType, timeDelta: Int, message: ToxFriendMessage)(state: ToxCoreState): ToxCoreState = {
  ...
  override def friendLossyPacket(friendNumber: Int, data: ToxLossyPacket)(state: ToxCoreState): ToxCoreState = {
    observers.foreach(_.friendLossyPacket(friendNumber, data)(()))
    next.friendLossyPacket(friendNumber, data)(state)
  }

  override def fileRecv(friendNumber: Int, fileNumber: Int, kind: Int, fileSize: Long, filename: ToxFilename)(state: ToxCoreState): ToxCoreState = {
  ...
  override def friendRequest(publicKey: ToxPublicKey, timeDelta: Int, message: ToxFriendRequestMessage)(state: ToxCoreState): ToxCoreState = {
  ...
  override def fileRecvChunk(friendNumber: Int, fileNumber: Int, position: Long, data: Array[Byte])(state: ToxCoreState): ToxCoreState = {
  ...
  override def friendLosslessPacket(friendNumber: Int, data: ToxLosslessPacket)(state: ToxCoreState): ToxCoreState = {
    observers.foreach(_.friendLosslessPacket(friendNumber, data)(()))
    next.friendLosslessPacket(friendNumber, data)(state)
  }

  override def friendConnectionStatus(friendNumber: Int, connectionStatus: ToxConnection)(state: ToxCoreState): ToxCoreState = {
  ...
  override def fileRecvControl(friendNumber: Int, fileNumber: Int, control: ToxFileControl)(state: ToxCoreState): ToxCoreState = {
  ...
  override def friendStatusMessage(friendNumber: Int, message: ToxStatusMessage)(state: ToxCoreState): ToxCoreState = {
  ...
  override def call(friendNumber: Int, audioEnabled: Boolean, videoEnabled: Boolean)(state: ToxCoreState): ToxCoreState = {
    observers.foreach(_.call(friendNumber, audioEnabled, videoEnabled)(()))
    next.call(friendNumber, audioEnabled, videoEnabled)(state)
  }

  override def callState(friendNumber: Int, callState: util.Collection[ToxavFriendCallState])(state: ToxCoreState): ToxCoreState = {
    observers.foreach(_.callState(friendNumber, callState)(()))
    next.callState(friendNumber, callState)(state)
  }

  override def bitRateStatus(friendNumber: Int, audioBitRate: BitRate, videoBitRate: BitRate)(state: ToxCoreState): ToxCoreState = {
  ...
  override def audioReceiveFrame(friendNumber: Int, pcm: Array[Short], channels: AudioChannels, samplingRate: SamplingRate)(state: ToxCoreState): ToxCoreState = {
