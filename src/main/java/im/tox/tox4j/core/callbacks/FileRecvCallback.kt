...
interface FileRecvCallback<ToxCoreState> {
   ...
   * transfers start in the PAUSED state. After this callback returns, a transfer can be rejected by
   * sending a [[ToxFileControl.CANCEL]] control command before any other control commands. It can
   * be accepted by sending [[ToxFileControl.RESUME]].
   *
   ...
   * @param filename Name of the file. May not be the actual name. This name was sent along with the
   *   file send request.
   */
  fun fileRecv(
